...
function smarty_function_paginate_middle($params, &$smarty) {
    ...
    while($_item <= $_total) {
        ...
        if($_item != $_curr_item) {
            ...
            $_ret .= $_link_prefix . '<li><a href="' . str_replace('&', '&', $_this_url) . '"' . $_attrs . '><span class="page_num">' . $_text . '</span></a></li>' . $_link_suffix;
        } else {
            $_ret .= $_link_prefix . '<li class="current">' .$_text . '</li>' . $_link_suffix;
        }
        $_item += $_limit;
        $_page++;
        $_display_pages++;
