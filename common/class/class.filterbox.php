...
class FilterBox
{
	...
	function AddCustomFilter($value)
	{
		$filterItem = new FilterItem('',$value,'','',true);
...
class FilterItem
{
	...
	function __construct($field, $value, $operator, $logic_operator,$is_custom=null)
	{
		$this->Field = $field;
		$this->Value = $value;
    ...
    function Init()
    {
        ...
        switch($fieldName)
        {
			...
			case "email":
                foreach($searchWords as $searchWord)
                {
                    $this->AddFilter("email", "'%".$searchWord."%'", "LIKE", "AND");
                }
