...
class Img2Thumb	{
	...
	function NewImgCreate($filename,$newxsize,$newysize,$fileout)
	{
		if( file_exists ($filename))
		{
			...
			if (!empty($fileout))$this-> NewImgSave($new_img,$fileout,$type);
			else $this->NewImgShow($new_img,$type);
	...
	{
		$ext=strtolower(substr($fileout,strlen($fileout)-4,4));
		switch($type)
		{
			case "gif":
			...
			case "jpg":
				if ($ext !=".jpg") $fileout .= ".jpg";
				return imagejpeg($new_img,$fileout);
				break;
			case "png":
				if ($ext !=".png") $fileout .= ".png";
