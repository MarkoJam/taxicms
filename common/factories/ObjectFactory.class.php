...
class ObjectFactory extends factoryBase 
{
	...
	function createObject($class_name, $id=null, $class_array=array(), $selectAttributes = "*")
	{
		try
		{
			...
			if (is_numeric($id)) eval("\$this->object->set".$meta_class_info->getPrimaryKeyAttr()."(".$id.");");
			else eval("\$this->object->set".$meta_class_info->getPrimaryKeyAttr()."('".$id."');");
