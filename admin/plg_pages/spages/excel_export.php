<?
	...
	$objlist = $ObjectFactory->createObjects("StaticPage", array("SfStatus","SfPageType","Template"));
	
	$workbook = new Spreadsheet_Excel_Writer();
	$workbook->setVersion(8);

	$workbook->send('export-excel.xls');
	$worksheet =& $workbook->addWorksheet('StaticPages');
	...
	if(count($objlist)>0)
	{
		...
		{
			$cnt++;
			$worksheet->write($cnt, 0, $odo->getHeader());
			$worksheet->write($cnt, 1, $odo->getTemplate()->Title);
