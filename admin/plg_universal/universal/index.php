<?
	...
	if($auth->isActionAllowed("ACTION_UNIVERSAL_VIEW"))
	{
		$ap = new AdminTable();
		$ap->SetOffsetName("offset_universalplugin");
		...
		$ap->SetCountAllRows($DBBR->prebrojSveSlogove($ObjectFactory->createObject("UniversalPlugin",-1), $ObjectFactory->filters));

		$ObjectFactory->AddLimit($ap->GetRowCount());
		$ObjectFactory->AddOffset($ap->GetOffset());
		$ObjectFactory->ManageSort();

		$objlist = $ObjectFactory->createObjects("UniversalPlugin");
