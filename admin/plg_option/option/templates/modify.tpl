	...
	<form  id='inner' action="modify_final.php" method="post" enctype="multipart/form-data" >
		...
		<div class="row">
            <div class="col-lg-12">
                        ...
                        <div id="tab-1" class="tab-pane  {if $active ne 3}active{/if}">
                            <div class="panel-body">
                                <fieldset class="form-horizontal">
									...
									<div class="form-group"><label class="col-sm-2 control-label">{$PLG_MODULES}</label>
										<div class="col-sm-10">
											<select name="moduleid" class="form-control">
												{html_options values=$module_val selected=$module_sel output=$module_out}
											</select>
										</div>
									</div>
									{*<div class="form-group"><label class="col-sm-2 control-label">{$PLG_CATEGORY}</label>
										<div class="col-sm-10">
											...
											<select size="5" multiple="multiple" name="optioncategories[]" class="chosen-select">
												{html_options values=$optioncategory_val selected=$optioncategory_sel output=$optioncategory_out}
											</select>
											</div>
										</div>
									</div>*}
				...
				<input name="option_id" type="hidden" id="option_id" value="{$optionid}">
			</div>
		</div>
	</form>
</div>
