<? 
	...
	{
		if(($_REQUEST["mode"])=='insert') $_REQUEST["option_id"]=-1;
		...
		{
			$upit ="SELECT * FROM audio WHERE class='option' AND `id` = ".$_REQUEST["option_id"] ;
			...
			$nw = $ObjectFactory->createObject("Option",$option_id, array("OptionCategory"));
			if (isset($_REQUEST['header'])) $nw->Option_POST($_POST);
			...
			// statusi
			$ObjectFactory->AddFilter(" tip_status_id = " . STATUS_TIP_OPTION);
			...
			// modul
			$ObjectFactory->AddFilter(" status_id = " . STATUS_MODULE_AKTIVAN);
			$modules = $ObjectFactory->createObjects("Module");
			
			$shModule = new SmartyHtmlSelection("module",$smarty);
			foreach ($modules as $m) 
			{
				$shModule->AddOutput($m->getHeader());
				$shModule->AddValue($m->getModuleID());
			}
			$shModule->AddSelected($nw->Module->getModuleID());
			$shModule->SmartyAssign();
			$ObjectFactory->ResetFilters();
			...
			$nws_selected = array($nw->getOptionType()->getOptionTypeID());
			
			$optiontypes= $ObjectFactory->createObjects("OptionType");
			
			foreach($optiontypes as $nws)
			{
				array_push($nws_output, $nws->getTitle());
				array_push($nws_values, $nws->getOptionTypeID());
			}
			...
			$optionCategories = $ObjectFactory->createObjects("OptionCategory");
				
			// select box za Kategoriju
			$shkateg = new SmartyHtmlSelection("optioncategory",$smarty);
			foreach($optionCategories as $nwc)
			{
				$shkateg->AddValue($nwc->getOptionCategoryID());
				$shkateg->AddOutput($nwc->getTitle());
			}
			if (isset($_REQUEST['cnc']) && $_REQUEST['cnc']==1) {
				$optioncategories = explode(',',$_POST["optioncategories"]);
				foreach($optioncategories as $kateg)
				{								
					$shkateg->AddSelected($kateg);
				}
			}		
			else {
				if(count($nw->OptionCategory) > 0)
				{
					foreach($nw->OptionCategory as $pk)
					{
						$shkateg->AddSelected($pk->getOptionCategoryID());
					}
				}
			}
			...
			// vezani resursi
			// priprema potencijalnih vezanih resursa
			$arrMP=array();
			$ObjectFactory->ResetFilters();
			$resres= $ObjectFactory->createObjects("ResRes");
			$ObjectFactory->ResetFilters();	
			foreach($resres as $rr) {
				$r1=explode('.',$rr->getResID());
				if ($r1[0]==7) {
					if ($r1[1]==$nw->Module->getModuleID()) {
						$r2=explode('.',$rr->getConResID());
						if ($r2[0]==1) {
							$arrMP[]=$r2[1];
						}	
					}
			...
			$ObjectFactory->ResetFilters();
			$ObjectFactory->AddFilter("class = 'PrProizvod'"  );			
			$resources= $ObjectFactory->createObjects("SfResource");
			$ObjectFactory->ResetFilters();

			// select box za Option
			...
			foreach($resources as $res) {
				...
				{
					if (in_array($rs->getProizvodID(),$arrMP)) {
			...
			$ObjectFactory->ResetFilters();
			$ObjectFactory->AddFilter("class = 'Option'"  );			
			$resources = $ObjectFactory->createObjects("SfResource");
			$ObjectFactory->ResetFilters();
			...
			$smarty->assign("img_rows",$modify->ModifyConnectedObject('Option', 'img', $_REQUEST["option_id"]));
			$smarty->assign("vid_rows",$modify->ModifyConnectedObject('Option', 'vid', $_REQUEST["option_id"]));
			$smarty->assign("web_rows",$modify->ModifyConnectedObject('Option', 'web', $_REQUEST["option_id"]));		
			$smarty->assign("doc_rows",$modify->ModifyConnectedObject('Option', 'doc', $_REQUEST["option_id"]));
