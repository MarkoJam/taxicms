	...
	function modify_plugin() {
		...
		$(document).ready(function() {
			...
			$('.title-actionconres #insert_conres').click(function() {
				...
				var param = 'conres_id='+crid+'&res_id='+nid+'&class=News'+'&mode='+mode;
				if (mode=='insert') mode='insert2';
				window.param2='mode='+mode+'&news_id='+nid+'&active=3'+param_add() + '&cnc=1';
				update2(link,param);
			})
	...
	<form  id='inner' action="modify_final.php" method="post" enctype="multipart/form-data" >
		...
		<div class="row">
            <div class="col-lg-12">
                            ...
                            <div class="panel-body">
                                <fieldset class="form-horizontal">
									...
									<div class="form-group"><label class="col-sm-2 control-label">{$PLG_CATEGORY}</label>
										<div class="col-sm-10">
											<div>
											<select size="5" multiple="multiple" name="newscategories[]" class="chosen-select">
												{html_options values=$newscategory_val selected=$newscategory_sel output=$newscategory_out}
											</select>
											</div>
										</div>
									</div>
				...
				<input name="news_id" type="hidden" id="news_id" value="{$newsid}">
			</div>
		</div>
	</form>
</div>
