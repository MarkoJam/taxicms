<?
	...
	if($auth->isActionAllowed("ACTION_NEWS_CATEGORY_VIEW"))
	{
		...
		$ap->SetOffsetName("offset_newscategory");
		$ap->SetCountAllRows($DBBR->prebrojSveSlogove($ObjectFactory->createObject("NewsCategory",-1), $ObjectFactory->filters));

		$ObjectFactory->AddLimit($ap->GetRowCount());
		$ObjectFactory->AddOffset($ap->GetOffset());

		$objlist = $ObjectFactory->createObjects("NewsCategory");
		...
		foreach($objlist as $nwc)
		{
			...
			{
				$modify_link = "<a class='naziv' href='modify.php?newscategoryid=".$nwc->NewsCategoryID."'>".$html_img_edit."</a>";
			}
			else
			{
			...
			{
				$delete_link = "<a href='delete_final.php?newscategoryid=".$nwc->NewsCategoryID."'>".$html_img_delete."</a>";
			}
			else
			{
	...
	else
	{
		...
		$smarty->assign("norights_message",$LanguageArray["value"]["PLG_NEWS_CATEGORY_NORIGHT_MODIFY"]);
