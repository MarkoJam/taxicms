<?
	...
	// f-ja dole
	if($this->usernameExists($validation->username ) && ($_REQUEST["mode"]=='insert')) $is_Valid = false;
	if($validation->password != $validation->passwordrep) $is_Valid = false;

	if(!$is_Valid)
	{
		// deo za obavestenje o neovlascenom upadu
	}
	else
	{		
		...
		if ($_REQUEST["mode"]!='insert') $user->SfStatus->setStatusID(STATUS_USER_AKTIVAN);
		else
		...
		{	
			...
			echo $message =  "Registrovao se novi korisnik na sajt: ". $user->Name . " " .$user->Surname . " , proverite u administraciji njegov status.";
